...
class Test_gpx_utilities(unittest.TestCase):
    ...
    def test_decode_gpx_plotaroute(self):
        ...
        gpx_path = os.path.join(os.path.dirname(os.path.split(os.path.abspath(__file__))[0]),'testData','PAR.gpx')
        decoded_route = gpx_utilities.decode_gpx_plotaroute(gpx_path)

        routeAsAList = list(zip(decoded_route['latitude'], decoded_route['longitude'], decoded_route['Instruction'], decoded_route['name'], decoded_route['altitude']))
