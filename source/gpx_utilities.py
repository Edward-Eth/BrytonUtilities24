...
def decode_gpx_plotaroute(gpx_path):
    ...
    for line in gpx_file:
        if line.find('lat=') != -1:
            ...
            tim = datetime.fromisoformat(
                line.lstrip().removeprefix('<time>').removesuffix('</trkpt>').removesuffix('</time>')).timestamp()
            time.pop()
            time.append(tim)
    ...
    for i, point in enumerate(sorted_zip):
        if (point[3] != 'none' or point[4] != 'none') or (i == 0 and (point[0] != sorted_zip[i + 1][0])) or (
                point[0] != sorted_zip[i - 1][0] and point[0] != sorted_zip[i + 1][
                0]):  # If the Point has an Instruction, keep it
    ...
    decoded_gpx = {"latitude": stripped_latitude,
